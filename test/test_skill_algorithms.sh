...
# Tests: E-ANCHORS-DETERMINISTIC-RESOLUTION, E-ANCHORS-CANONICAL-IDS,
#        E-ANCHORS-SMART-ROUTING, E-ANCHORS-IDEMPOTENT-OPS,
#        E-ANCHORS-AGENT-AGNOSTIC, E-ANCHORS-RESEARCH-PROTECTION,
#        E-ANCHORS-CHECK-COMPLETENESS
# Validates that SKILL.md instructions contain all algorithm steps
# required by ENGINEERING.md, so the LLM has complete guidance.
set -euo pipefail
source "$(dirname "$0")/helpers.sh"

echo "  [1] E-ANCHORS-DETERMINISTIC-RESOLUTION: Truth hierarchy is fully documented"
# Must show: PRODUCT.md > ENGINEERING.md > DEPENDENCIES.md, TESTING.md covers both, Tests > impl, Docs > tests
assert_grep "SKILL.md states PRODUCT.md is authoritative" 'PRODUCT\.md.*authoritative|PRODUCT\.md is authoritative' "$SKILL_FILE"
...
echo "  [2] E-ANCHORS-DETERMINISTIC-RESOLUTION: All 5 disagreement rules present"
...
# Rule 2: tests vs PRD/Engineering → fix tests
assert_grep "Rule: tests vs docs" 'tests.*PRD.*fix tests|Tests vs.*documents.*fix tests|Tests vs PRD' "$SKILL_FILE"
# Rule 3: TESTING.md vs PRD/Engineering → fix TESTING.md
assert_grep "Rule: TESTING.md vs PRD/Engineering" 'TESTING\.md.*PRD.*fix TESTING|TESTING\.md vs PRD' "$SKILL_FILE"
# Rule 4: ENGINEERING.md vs PRD → fix ENGINEERING.md
assert_grep "Rule: ENGINEERING.md vs PRD" 'ENGINEERING\.md vs PRD.*ENGINEERING\.md is wrong|ENGINEERING\.md.*PRD.*authoritative' "$SKILL_FILE"
# Rule 5: DEPENDENCIES.md vs ENGINEERING.md → investigate
assert_grep "Rule: DEPS vs ENGINEERING" 'DEPENDENCIES.*ENGINEERING.*check|DEPENDENCIES\.md vs ENGINEERING' "$SKILL_FILE"

echo "  [3] E-ANCHORS-CANONICAL-IDS: Code tagging convention documented"
...
echo "  [4] E-ANCHORS-SMART-ROUTING: 3-step path resolution"
...
echo "  [5] E-ANCHORS-IDEMPOTENT-OPS: Existing file handling"
...
echo "  [7] E-ANCHORS-RESEARCH-PROTECTION: Default name and prefix suggestions"
...
echo "  [8] E-ANCHORS-CHECK-COMPLETENESS: Module discovery with exclusions"
...
echo "  [9] E-ANCHORS-SMART-ROUTING: Recommendation based on existing modules"
